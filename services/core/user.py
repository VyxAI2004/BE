...
class UserService(BaseService[User, UserCreate, UserUpdate, UserRepository]):
    ...
    def promote_user_to_admin(
        ...
        if role_slug not in ["admin", "super_admin"]:
            raise ValueError(
                "Can only promote to 'admin' or 'super_admin' roles"
            )
        ...
        if self.role_repository.check_user_has_role(
            user_id=str(user_id), role_id=str(target_role.id)
        ):
            raise ValueError(f"User already has '{role_slug}' role")

        admin_roles = self.role_repository.get_admin_roles(
            slugs=["admin", "super_admin"]
        )
        admin_role_ids = [str(role.id) for role in admin_roles]
        self.repository.remove_admin_roles(
            user_id=str(user_id), admin_role_ids=admin_role_ids
        )

        default_reason = (
            reason or f"Promoted to {role_slug} by super admin"
        )
        ...
        print(
            f"User {user.email} promoted to {role_slug} by user {promoted_by}"
        )
    ...
    def update_user(
        ...
        if payload_dict.get("password"):
            payload_dict["password_hash"] = hash_password(
        ...
        updated_user = self.update(
            db_obj=db_user, payload=payload_update
        )

        if updated_user and updated_user.urls:
            if isinstance(updated_user.urls, dict):
                updated_user.urls = (
                    updated_user.urls.get("items", [])
                    if "items" in updated_user.urls
                    else []
                )
    ...
    def get_user_roles(self, user_id: uuid.UUID) -> List[Role]:
        ...
        return self.role_repository.get_user_roles(user_id=user_id)

    def count_users(
        self, *, filters: Optional[UserFilters] = None
    ) -> int:
